...
function memberOk() {
    ...
    var mode="${mode}";
    if(mode=="created") {
    	f.action = "<%=cp%>/member/memberM_ok.do";
    } else if(mode=="update") {
    	f.action = "<%=cp%>/member/updateM_ok.do";
    }
...
function userIdCheck() {
	...
	$.ajax({
		...
		,success:function(data){
			var p=data.passed;
			if(p=="true"){
				var s="<span style='color:blue; font-weight:bold;'>"+userId+"</span> 아이디는 사용가능합니다.";
...
<div class="container">
    <div class="body-container" style="width: 700px;">
        <div class="body-title">
            <h3><span style="font-family: Webdings">2</span>  ${title=='회원정보수정'?"개인회원 정보수정":"개인회원 가입"}  </h3>
        </div>
