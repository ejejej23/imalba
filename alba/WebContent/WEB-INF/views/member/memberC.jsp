...
function memberOk() {
    ...
    if(mode=="created"||mode=="update"&&f.upload.value!=""){
    	if(!/(\.gif|\.jpg|\.png|\.jpeg)$/i.test(f.upload.value)){
    		alert("로고이미지를 선택해주세요!! 이미지파일만 업로드가능!!");
    		f.upload.focus();
    		return;
    	}
    }
    ...
    if(mode=="created") {
    	f.action = "<%=cp%>/member/memberC_ok.do";
    } else if(mode=="update") {
    	f.action = "<%=cp%>/member/updateC_ok.do";
    }
...
function comIdCheck() {
	var comId = $("#comId").val();
	if(!/^[a-z][a-z0-9_]{4,9}$/i.test(comId)) { 
		var str = "아이디는 5~10자이내 , 첫글자는 영문자입니다.";
		$("#comId").focus();
		$("#comId").parent().next(".help-block").html(str);
		
		return;
	}
	
	var url = "<%=cp%>/member/comIdCheck.do";
	var query = "comId="+comId;
	$.ajax({
		type:"post"
		,url:url
		,data:query
		...
		,success:function(data){
			var p=data.passed;
			if(p=="true"){
				var s="<span style='color:blue; font-weight:bold;'>"+comId+"</span> 아이디는 사용가능합니다.";
				$("#comId").parent().next(".help-block").html(s);
			}else{
				var s="<span style='color:red; font-weight:bold;'>"+comId+"</span> 아이디는 사용할 수 없습니다.";
				$("#comId").parent().next(".help-block").html(s);
				$("#comId").val("");
				$("#comId").focus();
			}
...
<div class="container">
    <div class="body-container" style="width: 700px;">
        <div class="body-title">
            <h3><span style="font-family: Webdings">2</span> ${title=='회원정보수정'?"기업회원 정보수정":"기업회원 가입"} </h3>
        </div>
        ...
        <div>
			<form name="memberForm" method="post" enctype="multipart/form-data">
			  ...
			  <tr>
			      ...
			      <td style="padding: 0 0 15px 15px;">
			        <p style="margin-top: 1px; margin-bottom: 5px;">
			            <input type="text" name="comId" id="comId" value="${dto.comId}"
                         onchange="comIdCheck();" style="width: 95%;"
			  ...
			  <tr>
			      ...
			      <td style="padding: 0 0 15px 15px;">
			        <p style="margin-top: 1px; margin-bottom: 5px;">
			            <input type="password" name="comPwd" maxlength="15" class="boxTF"
			  ...
			  <tr>
			      ...
			      <td style="padding: 0 0 15px 15px;">
			        <p style="margin-top: 1px; margin-bottom: 5px;">
			            <input type="password" name="comPwdCheck" maxlength="15" class="boxTF"
			  ...
			  <tr>
			      <td width="100" valign="top" style="text-align: right; padding-top: 5px;">
			            <label style="font-weight: 900;">회사명</label>
			      </td>
			      <td style="padding: 0 0 15px 15px;">
			        <p style="margin-top: 1px; margin-bottom: 5px;">
			            <input type="text" name="comName" value="${dto.comName}" maxlength="30" class="boxTF"
		                       style="width: 95%;"
		                      ${mode=="update" ? "readonly='readonly' ":""}
		                      placeholder="이름">
			        </p>
			  ...
			  <tr>
			      <td width="100" valign="top" style="text-align: right; padding-top: 5px;">
			            <label style="font-weight: 900;">담당자 이메일</label>
			      </td>
			      <td style="padding: 0 0 15px 15px;">
			        <p style="margin-top: 1px; margin-bottom: 5px;">
			      ...
			      <td style="padding: 0 0 15px 15px;">
			        <p style="margin-top: 1px; margin-bottom: 5px;">
			            <select class="selectField" id="tel1" name="tel1" >
			                <option value="">선 택</option>
			                <option value="017" ${dto.tel1=="02" ? "selected='selected'" : ""}>02</option>
			                <option value="018" ${dto.tel1=="031" ? "selected='selected'" : ""}>031</option>
			                <option value="019" ${dto.tel1=="032" ? "selected='selected'" : ""}>032</option>
			  ...
			  <tr>
			      ...
			      <td style="padding: 0 0 15px 15px;">
			        <p style="margin-top: 1px; margin-bottom: 5px;">
			            ...
			            <button type="button" class="btn" onclick="searchZip()" >우편번호</button>          
			        </p>
			      </td>
			  </tr>
			  ...
			  <tr>
			      <td width="100" valign="top" style="text-align: right; padding-top: 5px;">
			            <label style="font-weight: 900;">회사 주소</label>
			      </td>
			      <td style="padding: 0 0 15px 15px;">
			        <p style="margin-top: 1px; margin-bottom: 5px;">
			            <input type="text" name="addr1" id="addr1" value="${dto.address1}" maxlength="50" 
			                       class="boxTF" style="width: 95%;" placeholder="기본 주소" readonly="readonly">
			        </p>
			        <p style="margin-bottom: 5px;">
			            <input type="text" name="addr2" id="addr2" value="${dto.address2}" maxlength="50" 
			  ...
			  <tr>
			      <td width="100" valign="top" style="text-align: right; padding-top: 5px;">
			            <label style="font-weight: 900;">로고이미지</label>
			      </td>
				...
				<c:if test="${mode=='update' }">
					<tr>
						<td width="100" valign="top"
							...
							style="font-weight: 900;">로고이미지</label></td>
						<td style="padding-left: 10px;"><img
							src="<%=cp%>/uploads/photo/${dto.logoImage}" width="30"
							height="30" onclick="viewerImage();" style="cursor: pointer;">
						</td>
					</tr>
				</c:if>
			  ...
			  <table style="width: 100%; margin: 0px auto; border-spacing: 0px;">
			     <tr height="45"> 
			      <td align="center" >
			        ...
			        <button type="button" class="btn" onclick="javascript:location.href='<%=cp%>/main.do';">${mode=="created"?"가입취소":"수정취소"}</button>
			      </td>
			    </tr>
			    <tr height="30">
