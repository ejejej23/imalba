...
public class FileManager {
	...
	public static boolean doFiledownload(String saveFilename, String originalFilename, String pathname,
		...
		if (pathname == null || saveFilename == null || saveFilename.length() == 0 || originalFilename == null
				|| originalFilename.length() == 0) {
			return flag;
		}

		try {
			// 자바가 euc-kr로 저장되고 있어서 이걸로 써줌. 만약 utf-8방식을 쓴다면 utf-8이라고 써줘야함
			...
			// 파일명 전송 : 파일명은 헤더에 정보를 실어서 보낸다.
			resp.setHeader("Content-disposition", "attachment;filename=" + originalFilename);

			// 클라이언트에게 파일의 내용을 전송
			byte[] b = new byte[1024];
			// BufferedInputStream으로 감싸지 않고 FileInputStream를 써도 되지만 BufferedInputStream로
			// 감싸면 속도가 더 빠르게 처리될 수 있다.
			BufferedInputStream bis = new BufferedInputStream(new FileInputStream(f));

			// 클라이언트에게 전송할 출력 스트림
			// 파일전송시 printWriteStream 등 필터스트림을 쓰면 파일이 깨질 수 있음
            // 반드시 OutputStream을 사용해야함
