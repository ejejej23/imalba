...
public class ApplyServlet extends MyServlet {
	...
	private void listApply(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		List<ApplyAdDTO> list = null;

		int listNum, n = 0;
		// 관리자
		if (searchValue.length() == 0) {
			list = dao.listApplyA(start, end);
		} else {
			// 검색
			list = dao.listApplyA(start, end, searchKey, searchValue);
		}

		String temp, temp2;
		for (ApplyAdDTO dto : list) {
		...
		forward(req, resp, "/WEB-INF/views/applyAdmin/list.jsp");
	}
